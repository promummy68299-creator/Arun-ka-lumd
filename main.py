...
def build_user_plans_keyboard():
    ...
    # Dono side extra padding aur arrows laga diye hain taaki buttons exact viral bot jaise poore lambe dikhein
    for p in plans:
        btn_text = f"🔹 {p[1]} — ₹{p[2]} / {p[3]} 🔹"
        btn_plan = types.InlineKeyboardButton(btn_text, callback_data=f"buy_now_{p[0]}")
        markup.add(btn_plan)
        
    # Bottom dynamic navigation layout 
...
# FIXED CORE ROUTER: Ab adm_, payset_ aur approvals ke saare buttons responsive ho gaye hain
@bot.callback_query_handler(func=lambda call: any(call.data.startswith(x) for x in ["adm_", "payset_", "ap_pay_", "rj_pay_"]))
def handle_admin_callbacks(call):
    ...
    if action == "adm_create_plan":
        msg = bot.send_message(call.message.chat.id, "1️⃣ Enter **Plan Name** text (Aap yahan apna Emoji add kar sakte hain):")
        ...
        bot.send_message(call.message.chat.id, "⚙️ Dynamic settings running securely on top of Railway.")
        
    elif action == "payset_edit_upi":
        ...
        try:
            ...
            bot.send_message(call.message.chat.id, f"❌ QR Core exception layer processing mismatch: {e}")
